                ...
                {classes.map((classItem) => (
                    <div key={classItem.id} className="flex items-center px-2 rounded-lg py-4 w-full justify-between h-16 gap-2 mt-5" style={{ backgroundColor: classItem.bg }}>
                        <div className="flex flex-row items-center gap-2">
                            <img src={classItem.img} className="md:size-8 size-6 lg:size-8" alt="Class Icon" />
                            <div className="flex flex-col">
                                <p className="md:text-sm text-xs" style={{ color: classItem.accentL }}>{classItem.grade}, {classItem.subject} | {classItem.type}</p>
                                <p className="font-semibold text-xs md:text-md lg:textmd" style={{ color: classItem.accentH }}>{classItem.day}, {classItem.time}</p>
                            </div>
                        </div>
                        <div>
                            <p className="md:text-sm text-xs" style={{ color: classItem.accentL }}>{classItem.date}</p>
                        </div>
                    </div>
                ))}
                </div>
